...
"""Unit tests for program structure and main flow."""
import os
import pathlib
import unittest
...
class StructureTesting(unittest.TestCase):
    ...
    def test_pyhyb_main(self):
        ...
        try:
            with patch(
                ...
                pyhyb_main(
                    input_file='test_files/test.json'
                )
            self.assertIn(
                'D-O-N-E', stdout.getvalue().strip()
            )
    ...
    def test_polite_functions(self):
        ...
        with patch(
            ...
            bye_world()
        self.assertIn(
            'execution', stdout.getvalue().strip()
        )
